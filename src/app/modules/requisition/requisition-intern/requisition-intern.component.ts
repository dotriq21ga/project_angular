...
import {
  ActionEnum, BranchResult, COMPARISION_OPERATOR, DATA_PROCESS,
  Filter, HEADER_TITLE_REQUISITION, IProcessStatus, ITeamPosition, IteamRequisition,
  Position, RequisitionConfigDiaLog, Skill, SortDirection,  StatusResult, ToastMessageType
...
import { FormDialogInternComponent } from './form-dialog-intern/form-dialog-intern.component';
import { FIRST_PAGE, IHeader, OPTION_PAGE, ROWS_PAGE } from 'src/app/shared/type/shared';

@Component({
  selector: 'app-requisition-intern',
  templateUrl: './requisition-intern.component.html',
  styleUrls: ['./requisition-intern.component.scss'],
  providers: [ConfirmationService, MessageService, DialogService]
})

export class RequisitionInternComponent {
  ...
  showDiaLog(action: ActionEnum, data?: IteamRequisition) {
    const requisitionIntern = action == ActionEnum.UPDATE ? data : null;
    const dialogConfig: RequisitionConfigDiaLog<IteamRequisition>
      = { requisition: requisitionIntern as IteamRequisition, action: action };
    this.ref = this.dialogService.open(FormDialogInternComponent, {
